...
class bloqueo_waf extends StatefulWidget {
  final Widget child;

  bloqueo_waf({Key key, this.child}) : super(key: key);

  _bloqueo_wafState createState() => _bloqueo_wafState();
}

class _bloqueo_wafState extends State<bloqueo_waf> {
  //
  Future recibirListaXSS() async {
    ...
    var respuesta;
    if(tipo_usuario == '1'){
      respuesta = await http.get(linkServerMaster+"/api/v1/get_lista_bloqueo_waf.php");
    } else {
      var url = linkServerMaster+"/api/v1/get_lista_bloqueo_waf_host.php";
